...
RUN apt-get install -y software-properties-common curl build-essential locales \
...
RUN pecl download mailparse && \
    ...
    cd mailparse-3.0.2 && phpize && ./configure && \
    sed -i 's/^\(#error .* the mbstring extension!\)/\/\/\1/' mailparse.c && \
    make && make install

RUN echo "extension=mailparse.so" > /etc/php/7.2/fpm/conf.d/30-mailparse.ini && \
...
# install gem
RUN gem install compass breakpoint susy
